...
void MapManager::triangulatePointsOnCamLCurKF()
{
    ...
    m_sgbm_ptr->compute(m_keyframes[m_curKFIdx].m_img_l, m_keyframes[m_curKFIdx].m_img_r, disparity_sgbm);
    ...
    for(int i = 0 ; i < m_keyframes[m_curKFIdx].m_kf_feats_l.size();i++)
    {
        ...
        int ptl_x = static_cast<int>(m_keyframes[m_curKFIdx].m_kf_feats_l[i].ptl.x());
        int ptl_y = static_cast<int>(m_keyframes[m_curKFIdx].m_kf_feats_l[i].ptl.y());
        if(ptl_x < 0 || ptl_x >= disparity.cols || ptl_y < 0 || ptl_y >= disparity.rows)continue;
        float disparity_value = disparity.at<float>(ptl_y, ptl_x);
        if(disparity_value <= 0.0f)continue;
        double depth = 0.301922 * 833.8 / disparity_value;
